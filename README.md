...
本项目以 **GNU Affero General Public License v3.0（`AGPL-3.0-only`）** 发布，完整条款见 [`LICENSE`](./LICENSE)，第三方名称与商标说明见 [`NOTICE`](./NOTICE)。

简单说清楚它意味着什么：

| 你可以 | 你必须 |
| :--- | :--- |
| 自由使用、修改、自建、自托管 | 分发修改版，或**把它做成网络服务对外提供**时，公开修改后的完整源码 |
| 用于个人或商业用途 | 保留原作者署名与许可声明（见下） |

依据 AGPL-3.0 第 7(b) 条，**任何分发或以网络服务方式对外提供本项目的衍生版本，均须在产品界面的「关于 / 法律声明 / 开源许可」页或随附文档的显著位置保留署名**：

```text
项目：LxAI — 本地 Agent 远程桥接
作者：lx00924-lx
源码：https://github.com/lx00924-lx/flutter-app
```

App 内已提供该界面：**设置 → GitHub 官方发布源 → 开源许可与署名**（同时列出全部第三方依赖的许可证）。

---

## 十、第三方名称与商标声明

- 本项目为**第三方独立开发**，与 DeepSeek（杭州深度求索人工智能基础技术研究有限公司）及其关联方**无任何隶属、合作、赞助或背书关系**。
- “DeepSeek”“DeepSeek Harness”及相关名称与标识归其各自权利人所有。本项目仅在**说明兼容性与互操作性**的范围内提及这些名称（指称性合理使用），不表示任何官方认证或授权。
- 本项目**不含**任何官方代码：本地能力通过 DeepSeek Harness 对外提供的运行时接口调用，未复制其源码。
